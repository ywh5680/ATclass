...
视图是 Django REST Framework 中处理 HTTP 请求和响应的核心组件。DRF 提供了多种视图类，从基础的 APIView 到高级的 ViewSet，满足不同的开发需求。本章将详细介绍如何创建和使用各种类型的视图。

### 基础视图

#### 1. APIView

APIView 是 DRF 中最基础的视图类，继承自 Django 的 View 类：
...
ViewSet 是 DRF 中最强大的视图类，它结合了多个视图的功能：
...
1. **基础视图**：APIView 和通用视图的使用
2. **视图集**：ModelViewSet、ReadOnlyModelViewSet 等
3. **高级功能**：过滤、搜索、分页、权限控制
4. **自定义操作**：使用 @action 装饰器添加自定义操作
5. **视图优化**：查询优化、缓存、异常处理
6. **视图测试**：编写完整的视图测试用例

视图是 DRF 中处理业务逻辑的核心组件。在下一章中，我们将学习如何配置 URL 路由来使用这些视图。 